...
Return valid JSON only: {"content":"supportive response","detectedEmotion":"stress|sadness|anxiety|loneliness|anger|happiness|neutral","aiSuggestion":"short practical step or null"}`;
      ...
      console.error(error);
      if (!res.headersSent) res.status(500).json({ error: "AI chat is unavailable. Check OPENAI_API_KEY and server configuration." });
      else { res.write(`data: ${JSON.stringify({ error: "AI chat is unavailable" })}\n\n`); res.end(); }
    }
  });
