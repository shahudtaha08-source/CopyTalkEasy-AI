...
function getDisabledAIMessage() {
  return {
    content: "AI Therapist is currently under development and unavailable in this release.",
...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
  ...
  app.post(api.chat.sendMessage.path, isAuthenticated, async (req: any, res) => {
    try {
      ...
      const disabledResponse = getDisabledAIMessage();
      await db.insert(messages).values({
        conversationId: id,
        role: "assistant",
        content: disabledResponse.content,
        detectedEmotion: disabledResponse.detectedEmotion,
        aiSuggestion: disabledResponse.aiSuggestion,
      });
      ...
      res.write(`data: ${JSON.stringify({ content: disabledResponse.content, detectedEmotion: disabledResponse.detectedEmotion, aiSuggestion: disabledResponse.aiSuggestion })}\n\n`);
      res.write(`data: ${JSON.stringify({ done: true })}\n\n`);
      res.end();
    } catch (error) {
      console.error("AI chat disabled:", error);
      if (!res.headersSent) {
        res.setHeader("Content-Type", "text/event-stream");
        res.setHeader("Cache-Control", "no-cache");
        res.setHeader("Connection", "keep-alive");
      }
      res.write(`data: ${JSON.stringify({ content: "AI Therapist is currently under development and unavailable in this release.", detectedEmotion: "neutral", aiSuggestion: null })}\n\n`);
      res.write(`data: ${JSON.stringify({ done: true })}\n\n`);
      res.end();
    }
  });
